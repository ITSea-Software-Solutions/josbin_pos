...
class EscPosBuilder {
  ...
  // Encode text as Latin-1 (covers Dutch special chars: ë é ij ó ú ñ)
  text(str: string): this {
    for (let i = 0; i < str.length; i++) {
      const code = str.charCodeAt(i)
      this.buf.push(code <= 0xff ? code : 0x3f) // '?' fallback for non-latin chars
    }
    return this
  }
  ...
  dashes(char = '-', width = 42): this {
    return this.line(char.repeat(width))
  }

  // Left text + right-aligned value in fixed width
  twoCol(left: string, right: string, width = 42): this {
  ...
  threeCol(qty: string, desc: string, price: string, width = 42): this {
    const qtyW  = 5
    const priceW = 10
...
export function buildReceiptBytes(opts: EscPosReceiptOptions): Uint8Array {
  ...
  if (parseFloat(sale.btw_srd) > 0) {
    ...
    b.twoCol(`${t.btw} 10%`, `SRD ${sale.btw_srd}`)
