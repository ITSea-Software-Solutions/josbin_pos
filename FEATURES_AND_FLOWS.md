...
> **Splitting the product into three nodes?** `migration-architecture-plan/20-split-build-plan.md`
...
| BTW-FILING-24 | Official **Belastingdienst Suriname government portal** — distinct gov-branded login at `/belastingdienst`, green/gold flag identity carried through sidebar chrome + 2FA screen | ✅ | tax_inspector | `BelastingdienstLoginScreen.tsx`, `theme/belastingdienst.ts`, role-aware `DashboardLayout` + `TwoFactorScreen` | — |
...
| CAT-17 | Barcode scanner — USB/Bluetooth HID (keyboard wedge). Enter-lookup accepts numeric 6–14 (UPC-E/EAN-8/UPC-A/EAN-13/ITF-14 + scale codes) and alphanumeric Code 39/128 SKUs; AIM prefixes (`]E0`…) stripped | ✅ | Cashier | `hooks/useBarcodeScanner.ts` (document-capture, speed-discriminated — focus-independent since 1.4.1) + `lib/barcode.ts` heuristics + `ProductGrid.tsx` | install guide §F0/F4 |
...
| HW-y | **Direct USB printing on Android** (USB Host API) — native `UsbPrinterPlugin` enumerates devices, requests the one-time OS permission, claims the printer interface and bulk-writes ESC/POS; device remembered by vendor+product so it survives replug/reboot. Removes the LAN-card / bridge-PC dependency for single-till stores; receipts AND drawer ride it | ✅ (since 1.4.1 — 1.4.0 shipped without the manifest USB-host declaration, so the device list was always empty on real hardware) | Cashier+ | `android/.../UsbPrinterPlugin.java`, `lib/usbPrinter.ts`, `lib/hardware.ts`, `SettingsScreen` | `docs/15 §15.9` |
...
| SET-13 | **Night / Day screen theme** (per till, not per user — a property of where the terminal stands and what light falls on it). Settings → Appearance; `:root[data-theme='day']` overrides the token block, so every screen follows without per-screen work. Day inverts the elevation stack (elevated LIGHTER than surface) and darkens the semantic colours for contrast on white; the orange accent is unchanged in both | ✅ | All | `index.css` day token block, `settingsStore.theme`, `App.tsx` theme effect, `SettingsScreen` Appearance | `user_manual/13 §13.3a` |
| SET-14 | **User menu on the POS top bar** — tap your name for role, store, language (NL/EN/SRN), on-screen keyboard (Windows), switch store, close register, log out. Replaces four loose bar controls that ran off the right edge of a 1024/1280 till and were unreachable (G-052). Exit guard still fires on a non-empty cart or open register | ✅ | All POS users | `components/pos/UserMenu.tsx` + `TopBar.tsx` | `user_manual/01 §1.5` |
| SET-15 | **Drawn product glyphs** — 15 category illustrations replace the single 📦 on tiles with no photo, matched on the store's OWN category names via regex in NL+EN. Bundled SVG paths, not fetched images: the till must work with no internet, and a photo set is tens of megabytes with licensing to track. A real uploaded product photo always wins | ✅ | All POS users | `components/pos/ProductGlyph.tsx` + `ProductCard.tsx` | `user_manual/04` |
...
- **2026-07-27 (Android USB printing — 1.4.0)** — User pushed back on "Android can't do USB printing": Posiflex's own software does it, so what's missing on our side? Nothing but code. Android has no raw print spooler, but it does have the **USB Host API** — which is exactly how Android POS software drives receipt printers. New native `UsbPrinterPlugin.java`: enumerate (`listDevices`, flagging devices with no bulk OUT endpoint as unprintable), one-time permission via PendingIntent (FLAG_IMMUTABLE for A12+, RECEIVER_NOT_EXPORTED for A13+), claim interface, chunked bulkTransfer of the same ESC/POS bytes every other platform gets — so the **cash drawer works through it too**. Devices are keyed by vendor+product, never Android's deviceId, so a replug doesn't lose the till's printer. Settings gains a USB device picker on Android (USB option un-hidden there, web stays network-only). This removes the LAN-card/bridge-PC dependency for single-till stores. Structurally verified in the APK (plugin in dex, UsbManager linked, JS bundled); **real-hardware confirmation pending** — that honesty is in the FEATURES row. Docs: ch15 §15.9 + capability table (EN+NL), release notes.
...
- **2026-07-27** — **1.4.1: the two office-floor faults, both ours.** (1) *USB printers invisible on Android.* `AndroidManifest.xml` never declared `<uses-feature android:name="android.hardware.usb.host" />` — mandatory per Android's USB-host guide — so `getDeviceList()` returned an empty map however the printer was cabled. 1.4.0's "verification" checked only that OUR code shipped (plugin in `classes8.dex`, `UsbManager` linked, JS bridge in the bundles); none of it checked the OS would let that code see anything. Fixed + a `USB_DEVICE_ATTACHED` intent-filter with a printer-class `device_filter.xml` so plugging a printer in offers to pair it, and `listDevices()` now returns `hostSupport`, device count and per-interface class/subclass/protocol + a plain reason, so an empty list names its own cause in the UI (three distinct messages, EN/NL/SRN). Verified with `aapt2 dump xmltree` against the built APK, not the source tree. (2) *"Have to scan twice", sluggish grid.* CAT-17 depended on the search input holding focus (G-048(b)); on a touch till every tap moves focus to the tapped tile, and re-focusing an input on Android opens the on-screen keyboard whose animation eats the opening characters of the next scan. Replaced with `hooks/useBarcodeScanner.ts`: document-level **capture-phase** listener that tells scanner from human by speed and swallows the scan so its Enter can't re-fire the tapped tile; judged on **average** gap (100ms/char budget) so one GC pause on a cheap ARM tablet can't silently drop a scan; focus-steering and `autoFocus` are now desktop-only. 13 new vitest (121 total), tsc clean. Proven in a real browser against the live catalogue: tapped Coca-Cola → scanned Butter's EAN → cart held Coca-Cola ×1 + Butter ×1, Enter confirmed swallowed, search box clean. Also fixed: every APK since 1.0.0 shipped `versionCode 1`, which Android can treat as a duplicate and refuse to install over — now derived from the version. Triggered by user: *"from pos i am not seeing printer not the scanner working properly ... sluggy exp, can u plz do proper investigation and come up with rigid fast expert solution"*. Lessons logged as G-050 in `CLAUDE_WORKING_GUIDE.md` (which supersedes G-048(b)).
