...
> **Batch 3 shipped 2026-06-05** — P0-6 + P1-S1…S7 resolved (security pass). Customer org-scope rule, webhook-secret encryption (col widened to text), svg-upload XSS, sanctum expiry backstop, void/refund throttle, 2FA middleware mounted, reset-2fa step-up, per-IP login throttle. Tests: `SecurityHardeningTest` (8).
> **Batch 4 shipped 2026-06-05** — **ALL P0 CLEAR.** P0-5 (receipt `?token=` → Bearer-blob + middleware rejects broad query tokens), P0-7 (WBP-S customer erasure `DELETE /customers/{id}` + dashboard Erase button), P0-8 (Customer PII events → hash-chained audit_logs: created/accessed/updated/redacted), P1-C4 (audit-log OA scope by org_id OR user), P1-C5 (locked daily-rate immutability + `withoutLockGuard` escape), P1-C6 (sale blocked w/o BTW number), P1-D2 (customer LTV exact DECIMAL). Tests: `CustomerErasureTest` (3), `ComplianceIntegrityTest` +3, `SecurityHardeningTest` +1. **Also:** fixed a Batch-3 2FA-middleware regression (enforce marker only on real PATs — 14 suite failures the security subset had missed); added `audit:rebaseline` + repaired the live chain that G-023's algorithm change had broken (now ✓ all orgs). Full suite **212 green**.
> **Remaining P1:** D1 (per-store sale_number sequence), D3 (refund discount), D5 (discount combine) — task #127; performance sweep P2–P6 (whereDate→whereBetween, unpaginated /products/pos, storeDetail N-queries, broadcast fan-out, recordSale batch); plus P2 items + i18n (XL).
