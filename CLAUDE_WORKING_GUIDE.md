...
- **2026-07-19 (morning recovery)** — Solved the "yesterday was never closed" support-call class (see the project thoughts discussion): stale previous-day sessions now route the POS register gate into a guided morning flow (manager counts+closes / cashier gets a tap-to-call+WhatsApp screen driven by store `manager_name`/`manager_phone`; cash figures manager-only in the `yesterday-status` endpoint), a per-store `closing_time` drives an amber POS strip + a once-daily `RegisterStillOpen` manager notification and a logout guard, and an opt-in nightly `registers:auto-close` (per-store `auto_close_enabled`+`auto_close_time`) seals forgotten sessions `system_closed` (status closed, expected cash computed, *not counted*) so mornings never block — the manager reconciles via `POST /sessions/{id}/reconcile` (a skippable task, discrepancy-note-required, audited `register.reconciled`). Store settings gained an **End of day** section (Dashboard → Store Settings). **G-036:** `(new SomeCommand)->handle()` in a test explodes at `$this->info()` — the command's `$this->output` is only bound when run through Artisan; call `$this->artisan('sig')->assertExitCode(0)` instead (hit while testing AutoCloseRegisters). `RegisterSession::computeExpectedCash()` now lives on the model (shared by the close endpoint + the auto-close command) — the controller's private copy delegates to it. +MorningRecoveryTest (6), backend 296; both SPAs tsc clean; cashier screen live-verified in the browser, manager cash-visibility via API. Docs: dashboard ch 19 §19.10 + user ch 3 (EN+NL).

- **2026-07-19 (ideas batch)** — WhatsApp receipts, Sranantongo (draft, 390 keys via subagent — native review list in the srn generation notes), WebAuthn passkeys, runtime POS server URL; internal portal gained OPS_CHEATSHEET.md + PARTNER_OUTREACH.md. Two new gotchas: **G-037** — in this Laravel version the *package* side of `mergeConfigFrom()` wins over `config/<name>.php` for keys both define (empirically proven: app file loaded, sentinel key visible, yet vendor's `allowed_origins` survived). Override package config in `AppServiceProvider::boot()` via `config([...])`, not by shadowing the file — that's where `passkeys.relying_party_id`/`allowed_origins` live now. **G-038** — `$this->actingAs($user, 'sanctum')` gives a `TransientToken`; any middleware touching `currentAccessToken()->expires_at` (session.timeout) explodes as an ErrorException under PHPUnit. Feature tests for routes in that group must mint a REAL token (`createToken()->plainTextToken` + `withToken()`); it's also truer to production. Also worth remembering: WebAuthn happy paths ARE testable headlessly — Playwright/CDP `WebAuthn.addVirtualAuthenticator` (ctap2 + internal + residentKey + UV) drove register+login against the demo stack; wipe `passkeys` between runs because each launch gets an empty authenticator. Vendor `Passkey` model guards `user_id` — create via the relation. `laravel/passkeys` challenge round-trip is caller-owned: cache the serialized options (`WebAuthn::toJson`) and `Cache::pull` them exactly once in the verify step (built-in replay kill).

- **2026-07-19 (pending-list clearance)** — EULA draft (legal/, internal portal, srcExcluded), customer detail view + statement (CUST-01/02), report caching (`ReportCache`, scope-keyed), and the June-audit D1/D3/D5 closed — see FEATURES §11 for the full entry; full suite 323/1096 post-merge. **G-037:** Sanctum's RequestGuard caches the resolved user on the app instance — a feature test that switches identities mid-test silently keeps the FIRST user; call `auth()->forgetGuards()` before each identity switch (hit in ReportCacheTest's scope tests). **G-038:** two `php artisan test` runs in the SAME app container share the test database — concurrent runs collide (`migrate:fresh` deadlocks, phantom cross-run failures that pass in isolation). Run suites serially per container; if a combined run shows failures that pass alone, suspect a parallel run before suspecting the code. **G-039:** `Sale::nextNumber()` now throws outside a DB transaction by design (P1-D1) — tests using RefreshDatabase are fine (the trait wraps each test in a txn); a new seeder or command calling it must open `DB::transaction` itself.

- **2026-07-25 (installer distribution + server address)** — Dashboard-served installer download (manager-gated, LAN-friendly, graceful not-deployed) + server-address panel; docs rewritten for the whole "which server does this till talk to" story. See FEATURES §11. **G-040:** the POS drives printer/cash-drawer from the TILL (`tcpPrint` in `electron/main.ts` opens the socket to the printer IP; USB via the OS) — the server is never in that path. So "remote server" does NOT disable hardware; only running in a *browser* does (`printEscPos` returns "not available in browser"). Don't repeat the earlier conflation of those two. **Release procedure (standing user rule, 2026-07-27):** exe and APK are ALWAYS released together with the SAME version, from the same commit. The single version source is `frontend/package.json` (also mirrored into `android/app/build.gradle` versionName). Per release: (1) bump version; (2) build BOTH artifacts with the droplet VITE_* env; (3) name them `josbin-pos-demo-Setup-<v>.exe` / `josbin-pos-demo-<v>.apk`, generate `.sha256` sidecars, rsync all four to `/var/www/html/downloads/`, checksum-verify remote; (4) confirm `GET /api/installer` reports the same version for both platforms; (4b) run `node scripts/build-downloads-page.mjs` — it reads the real files and rebuilds the public downloads page (versions/sizes/checksums can then never drift from what is on disk); (5) append a client-readable entry to `docs/17-release-notes.md` + NL mirror (no internal refs) and deploy docs. Never ship one platform without the other. **Ordering trap (2026-07-27):** the public page is SERVED from the VitePress dist (`/var/www/html/docs-site/.vitepress/dist/downloads.html` → docs-web's `/usr/share/nginx/html`), which `deploy-server.sh` populates. Editing `marketing/downloads.html` AFTER the deploy leaves the served copy stale — and if the artifact was renamed in between, the live download link 404s while the file sits happily on disk. So: regenerate the page BEFORE deploying, or rsync it straight into the dist path afterwards. Always finish by curl-ing the actual hrefs off the live page for `200`, not just the page itself — the page rendering fine is exactly what hides a broken link.

**G-050 (a native capability you never declared is a capability you don't have — and SUPERSEDES G-048(b)):** shipped 1.4.0 claiming "direct USB printing on Android", verified the plugin class was in `classes8.dex`, verified `UsbManager` was linked, verified the JS bridge was in all three bundles — and the device list was empty on real hardware every time. `AndroidManifest.xml` never declared `<uses-feature android:name="android.hardware.usb.host" />`, which Android's own USB-host guide states is mandatory; without it `UsbManager.getDeviceList()` returns an empty map no matter what is plugged in. **Every check I ran verified my code was present; none verified the OS would let it run.** For any native capability (USB, camera, Bluetooth, location, background work), the manifest declaration is part of the feature — grep the *built* manifest with `aapt2 dump xmltree --file AndroidManifest.xml <apk>`, not the source tree, and treat an empty enumeration result as "am I allowed to see this?" before "is anything connected?". Same class as G-043 (phantom dependency) and G-047 (false comment): the artifact contained the code, and the code could not work.

**G-051 (a transfer that moves ZERO bytes is a stalled endpoint, not a busy one — six releases lost to reading it as "busy"):** the Android cash drawer failed as `USB write failed after 0 to 5 bytes`. I prefixed the pulse with `ESC @` to make it a well-formed job; next report was `0 to 10 bytes`. **Identical failure, different payload — size was never the variable, and that was visible in the two error strings side by side before any more code was written.** A bulk transfer returning -1 having moved *zero* bytes after a full 5s timeout is a HALTED endpoint; a genuinely busy one moves a partial count or succeeds on retry. Halts do not clear with time, so every "sleep and retry" fix was structurally incapable of working. Root cause: `print()` force-claims the interface and closes it on *every* call, so the drawer's claim (sent ~1.2s after the receipt as a second job) landed while the printer was still physically printing, and a mid-job interface teardown stalls bulk OUT on much printer silicon. Receipts always worked because each was the first write on a fresh connection; Windows was never affected because its spooler holds one persistent handle and never re-claims. **Fixes, in order of importance:** (1) don't make a second USB transaction at all — the drawer pulse rides inside the receipt stream (`buildReceiptBytes`'s `openDrawer`), one claim, one write, nothing left to refuse; (2) recover rather than sleep — `CLEAR_FEATURE(ENDPOINT_HALT)` via `controlTransfer(0x02, 0x01, 0x00, ep.getAddress(), null, 0, …)`, then a full release/re-claim/clear, then tell the user to replug. **The decisive diagnostic came free from the user:** Settings → "Test cash drawer" fired the drawer perfectly on its own. Same bytes, same pin, same cable — the only difference in a sale is that the pulse *follows a receipt*. One works-in-isolation/fails-in-sequence data point isolates a sequencing bug faster than any amount of reasoning about payload contents; ask for it early. Related: G-047 (silent failure paths), G-050 (present ≠ able to run).

Two more from the same fix, both worth their own rule:

- *Scanner capture must not depend on focus (this REPLACES G-048(b)).* G-048(b) fixed the tapped-tile-steals-Enter bug by returning focus to the search input after every add. That is the wrong shape on a touch terminal: focusing a text input on Android opens the on-screen keyboard, whose slide-up animation swallows the opening characters of the next scan — the field symptom was "have to scan twice" plus a sluggish feel, i.e. the fix traded one scanner bug for another. Correct model: a HID scanner is told apart from a human by SPEED, captured at `document` in the **capture phase** (so `stopPropagation` keeps it out of focused buttons and inputs entirely), and never needs focus at all — `hooks/useBarcodeScanner.ts`. Judge the run on its **average** gap, not per-key: a single GC pause on a cheap ARM tablet would otherwise disqualify a real scan and silently reproduce the very bug. Focus-steering is now desktop-only.
- *`versionCode 1` forever.* Every APK from 1.0.0 to 1.4.0 shipped `versionCode 1`, because Capacitor's template sets it and versionName is what we were bumping. Android gates upgrades on versionCode, not versionName, so newer builds could be refused as duplicates. Now derived as `MAJOR*10000 + MINOR*100 + PATCH`. Rule: if a platform has its own version field, it must move on every release — a human-readable version string it never reads is not a release.

**G-047 (`print /D:` is not raw printing — second false-comment bug):** `usbPrint` shipped calling `print /D:"<printer>" file.bin` with a comment claiming it "sends the file directly as RAW job (bypasses GDI rendering)". It does not: `print` is a legacy TEXT utility, so ESC/POS control bytes never reached the printer — the field symptom was a Windows test page working fine while our receipts and drawer did nothing. Correct route is the spooler API with datatype `RAW` (OpenPrinter → StartDocPrinter → StartPagePrinter → WritePrinter), reached via PowerShell P/Invoke with `-EncodedCommand` (no temp .ps1 → ExecutionPolicy can't block it; no quoting issues with printer names containing spaces). Same lesson as G-043: **a comment asserting how a platform API behaves is not evidence** — for hardware paths, the only proof is paper coming out. Also shipped alongside: hardware test failures now surface the real spooler/socket message in the UI, because "red button, no reason" is unusable in the field.

**G-049 (a kiosk app must be closed BY the installer):** upgrades dead-ended on *"Josbin POS cannot be closed, please close it manually or retry"*. NSIS' default app-running check politely asks the user to close the app — but our till is fullscreen and frameless, so there is no window to close, and Electron runs several processes so killing one by hand isn't enough either. Fixed with `build/installer.nsh` overriding `customCheckAppRunning` (+ `customInit`) to `taskkill /F /IM "${APP_EXECUTABLE_FILENAME}" /T` — the `/T` matters, it takes the GPU/renderer/utility children too. Rule: **any kiosk-mode app must ship an installer that force-closes it**; "ask the operator to close the window" is not a strategy when the product's whole design is that there is no window chrome. Manual escape for machines still on an older build: PowerShell `Get-Process | ? { $_.Path -like "*Josbin*" } | Stop-Process -Force`, or reboot (and disable the auto-launch startup entry first, or it relaunches before the installer runs).

**G-048 (sandboxed renderers have no `Buffer`, and POS focus must live in the scan box):** two field bugs from the same office session. (a) `hardware.ts` sent USB print jobs as `Buffer.from(bytes)` while the network branch correctly sent `Array.from(bytes)` — the renderer runs with `sandbox: true` + `contextIsolation: true` + `nodeIntegration: false`, so `Buffer` is undefined there and the call threw before any IPC happened. Rule: **anything crossing the preload bridge must be a structured-cloneable plain value** (number[], not Buffer/Uint8Array-with-methods); when two branches of the same function serialize differently, one of them is wrong. Stacked on top of G-047, so the printer stayed silent for two independent reasons — fix one, still nothing prints; always re-test after each fix rather than assuming the first was the cause. (b) A hardware barcode scanner is a KEYBOARD: after a product tile is tapped the browser leaves focus on that button, so the scanner's trailing Enter re-fires the tile and increments the wrong line. Every add path now returns focus to the search input (rAF-deferred so React's re-render doesn't steal it back). Any new POS control that can take focus must return it the same way.

**G-046 (two nginx traps in one deploy):** (a) *single-file bind mounts pin an inode* — `git reset --hard` REPLACES `docker/frontends/docs.conf`, so the running container kept serving the old file even after `nginx -s reload`; only `docker compose up -d --force-recreate <svc>` picks it up. Any conf change shipped via git needs a force-recreate, not a reload. (b) *`return 302 /path` emits an ABSOLUTE Location* built from server_name + default port — on our non-standard port (8095) that silently redirected to port 80, a different site on the same host. Add `absolute_redirect off;` whenever redirecting inside a non-:80 server block. Both were caught by curl-ing the actual headers after deploy; neither would have shown up in a browser click-test that lands on *a* page.

**G-045 (the page-1-lie pattern):** the pagination audit found the same bug shape in six dashboard screens: fetch page 1 of a paginated endpoint with no `page` param, then run KPI tiles / search / bulk-select / sums over that page as if it were the whole set (UsersScreen counted "total users" over 25 rows; OrganisationsScreen's search box filtered client-side over one page — it actively lied). Rules that follow: (a) any client-controlled `per_page` MUST be clamped server-side (`min(…, 200)`) — otherwise "paginated" is a fiction; (b) KPI/summary numbers NEVER come from the rendered page — return them as `meta_counts`/`meta_totals` SQL aggregates next to the paginator; (c) a search box either sends `search` to the server or does not exist; (d) exports are period documents — cap the range, refuse oversized row counts with a "split the period" error, and chunk inside `streamDownload` (BTW-submissions export is the reference); (e) when a screen sends filter params, prove the backend consumes them — Laravel drops unknown request keys silently (the POS sales `date`/`search` filters were dead for months).

**G-044 (the third packaging-context bug this week — treat as a class):** config/cors.php allowed `capacitor://localhost` but Capacitor's androidScheme has defaulted to **`https://localhost` since v5**, so the Android WebView's preflight got 204-without-ACAO and every APK API call would have died — invisible in dev, where the origin is `http://localhost:5173`. Empirical scope check: Electron `file://` windows do NOT enforce CORS (a real login from a file:// harness against the unfixed droplet returned a token), so the exe was unaffected. The class of bug: **CSP (G-042), phantom plugin (G-043) and CORS (G-044) all only exist in the PACKAGED runtime context** — dev servers mask all three. Before shipping any packaged build, verify the trio from the artifact's own context: connect-src in the built index.html, plugin classes in the dex/asar, and a curl replay of the preflight with the packaged origin (`https://localhost` for Android; Electron sends none that matters). The 'null' origin is also allowed server-side now — harmless for a Bearer-token API, useful for other embedders.

**G-043 (phantom dependency):** the Android print path shipped for months importing `@capacitor-community/tcp-sockets` with comments claiming the package "is injected by the Capacitor runtime inside the compiled APK — not on npm". That is not a thing: Capacitor injects nothing; plugins are either real npm packages or locally registered classes. The import could never resolve, so on Android every receipt/drawer call would have silently fallen through to the Epson-only ePOS HTTP fallback and failed on non-Epson printers. Replaced with our own ~90-line `TcpSocketPlugin.java` registered in MainActivity + `registerPlugin('TcpSocket')` from `@capacitor/core`. Lessons: (1) a comment asserting how a dependency works is not evidence — if a package can't be found on npm, the code referencing it has never run; (2) when verifying an APK, `classes.dex` alone is not the app — debug builds split across classesN.dex, grep them all before concluding a class is missing; (3) Android LAN http needs BOTH `usesCleartextTraffic` (manifest) and mixed-content allowance — and the renderer CSP applies inside the Capacitor WebView exactly as in Electron (G-042's fix is what makes the APK able to talk to any server at all).

**G-042 (the expensive one):** the renderer CSP in `frontend/index.html` had `connect-src 'self' … http://localhost:*`, which in a PACKAGED build (loadFile → `file://` origin) blocks every non-localhost API call. Dev never showed it (vite serves from `http://localhost:5173`, so `'self'` + `localhost:*` covered the API), so the runtime server override, Reverb-over-HTTP and the published demo installer were all silently dead in production — the installer we had already put on the download page could not reach `142.93.88.143` at all. Proven with a real Electron `file://` window: `localhost` 200, same server via `127.0.0.1` blocked. Fixed by widening ONLY `connect-src` to `http: https:` (script/style/img stay `'self'` — that is where injection risk lives) with a why-comment above the meta tag. Lesson: an Electron app whose backend address is chosen by the operator cannot express its API host in a CSP allow-list; and dev-server testing NEVER exercises the packaged origin — verify renderer network policy in a file:// window before shipping an installer.
