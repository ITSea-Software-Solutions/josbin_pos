...
function EventBadge({ event }: { event: string }) {
  const cfg: Record<string, { bg: string; color: string; label: string }> = {
    ...
    updated: { bg: 'rgba(58,74,143,0.1)', color: '#3a4a8f', label: 'Updated' },
...
function ValuesDiff({ old: oldV, next: newV }: { old: Record<string, unknown> | null; next: Record<string, unknown> | null }) {
  if (!oldV && !newV) return <span style={{ color: '#7e88a0', fontSize: 12 }}>—</span>
  const keys = [...new Set([...Object.keys(oldV ?? {}), ...Object.keys(newV ?? {})])]
    .filter(k => !['updated_at', 'last_used_at', 'last_login_at'].includes(k))
  if (!keys.length) return <span style={{ color: '#7e88a0', fontSize: 12 }}>—</span>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      {keys.slice(0, 4).map(k => {
        ...
        return (
          <div key={k} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11 }}>
            <span style={{ color: '#7e88a0', fontWeight: 600, minWidth: 90, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{k}</span>
            ...
            {changed && <span style={{ color: '#7e88a0' }}>→</span>}
      ...
      {keys.length > 4 && <span style={{ fontSize: 10, color: '#7e88a0' }}>+{keys.length - 4} more fields</span>}
    </div>
  )
}
    ...
    <>
      ...
      >
        <td style={{ padding: '10px 16px', whiteSpace: 'nowrap' }}>
          <p style={{ fontSize: 12, fontWeight: 700, color: '#16203a' }}>{dateStr}</p>
          <p style={{ fontSize: 11, color: '#7e88a0' }}>{timeStr}</p>
        </td>
        ...
        <td style={{ padding: '10px 16px' }}>
          {entry.user ? (
            <div>
              <p style={{ fontSize: 12, fontWeight: 600, color: '#16203a' }}>{entry.user.name}</p>
              <p style={{ fontSize: 11, color: '#7e88a0' }}>{entry.user.role.replace('_', ' ')}</p>
            </div>
          ) : (
            <span style={{ fontSize: 12, color: '#7e88a0' }}>System</span>
          )}
        </td>
        ...
        <td style={{ padding: '10px 16px' }}>
          <span style={{ fontSize: 11, color: '#7e88a0', fontFamily: 'monospace' }}>
            {entry.ip_address ?? '—'}
          </span>
        </td>
        <td style={{ padding: '10px 16px', textAlign: 'center' }}>
          <span style={{ color: '#7e88a0', fontSize: 14 }}>{expanded ? '▲' : '▼'}</span>
        </td>
      </tr>
      {expanded && (
...
function RekenkamerPanel({ isNl }: { isNl: boolean }) {
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #ebebf0', padding: '20px 24px', marginBottom: 24, display: 'flex', flexWrap: 'wrap', alignItems: 'flex-end', gap: 16 }}>
      <div>
        <p style={{ fontSize: 13, fontWeight: 700, color: '#16203a', marginBottom: 2 }}>
          Rekenkamer van Suriname — {isNl ? 'Auditexport' : 'Audit Export'}
        </p>
        <p style={{ fontSize: 12, color: '#7e88a0' }}>
...
const labelSt: React.CSSProperties = { fontSize: 11, fontWeight: 600, color: '#7e88a0', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.5px' }
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: '100%' }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <h2 style={{ fontSize: 20, fontWeight: 800, color: '#16203a', letterSpacing: '-0.3px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 16, marginBottom: 24 }}>
          ...
          {Object.entries(summary.by_event).map(([evt, cnt]) => (
            <div key={evt} style={{ background: '#fff', borderRadius: 14, padding: '16px 20px', border: '1px solid #ebebf0', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
              <p style={{ fontSize: 24, fontWeight: 800, color: evt === 'created' ? '#059669' : evt === 'deleted' ? '#ef4444' : '#3a4a8f' }}>{cnt as number}</p>
              <p style={{ fontSize: 12, color: '#7e88a0', marginTop: 2, textTransform: 'capitalize' }}>{evt}</p>
            </div>
          ))}
          {Object.entries(summary.by_model).map(([model, cnt]) => (
            <div key={model} style={{ background: '#fff', borderRadius: 14, padding: '16px 20px', border: '1px solid #ebebf0', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
              <p style={{ fontSize: 24, fontWeight: 800, color: '#16203a' }}>{cnt as number}</p>
              <p style={{ fontSize: 12, color: '#7e88a0', marginTop: 2 }}>{model}</p>
            </div>
          ))}
        </div>
      )}
      ...
      <div style={{ background: '#fff', borderRadius: 14, padding: '16px 20px', border: '1px solid #ebebf0', marginBottom: 20, display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
        ...
        <div style={{ flex: '1 1 200px' }}>
          <p style={{ fontSize: 11, fontWeight: 600, color: '#5f6a84', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.5px' }}>{isNl ? 'Zoeken' : 'Search'}</p>
        ...
        {/* Event filter */}
        <div>
          <p style={{ fontSize: 11, fontWeight: 600, color: '#5f6a84', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Event</p>
          <select
            value={filters.event ?? ''}
        ...
        {/* Model filter */}
        <div>
          <p style={{ fontSize: 11, fontWeight: 600, color: '#5f6a84', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Model</p>
          <select
        ...
        {/* Date range */}
        <div>
          <p style={{ fontSize: 11, fontWeight: 600, color: '#5f6a84', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.5px' }}>{isNl ? 'Datum van' : 'Date from'}</p>
          <input type="date" value={filters.date_from ?? ''} onChange={e => setFilters(f => ({ ...f, date_from: e.target.value || undefined, page: 1 }))}
            style={{ height: 36, borderRadius: 8, border: '1px solid #e5e7eb', padding: '0 10px', fontSize: 13, background: '#fff', outline: 'none' }} />
        </div>
        <div>
          <p style={{ fontSize: 11, fontWeight: 600, color: '#5f6a84', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.5px' }}>{isNl ? 'Datum t/m' : 'Date to'}</p>
        ...
        <button
          ...
          style={{ height: 36, padding: '0 14px', borderRadius: 8, border: '1px solid #e5e7eb', background: '#fff', color: '#5f6a84', fontWeight: 600, fontSize: 13, cursor: 'pointer', alignSelf: 'flex-end' }}
        >
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tr style={{ background: '#f8f8fb', borderBottom: '2px solid #ebebf0' }}>
                  {[
                    ...
                    <th key={h.label || 'expand'} onClick={h.key ? () => toggleSort(h.key as string) : undefined}
                      style={{ padding: '10px 16px', textAlign: 'left', fontSize: 11, fontWeight: 700, color: '#5f6a84', textTransform: 'uppercase', letterSpacing: '0.5px', whiteSpace: 'nowrap', cursor: h.key ? 'pointer' : 'default', userSelect: 'none' }}>
                      {h.label}
              ...
              <tbody>
                ...
                {sortedEntries.length === 0 && (
                  <tr><td colSpan={7} style={{ textAlign: 'center', padding: 40, color: '#7e88a0', fontSize: 14 }}>
            ...
            {data && data.last_page > 1 && (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 20px', borderTop: '1px solid #f0f0f5' }}>
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  ...
                  <span style={{ height: 32, display: 'flex', alignItems: 'center', padding: '0 14px', fontSize: 13, color: '#5f6a84' }}>
