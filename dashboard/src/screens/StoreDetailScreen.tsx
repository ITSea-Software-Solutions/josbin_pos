...
export default function StoreDetailScreen({ storeId, onBack }: Props) {
  ...
  return (
    <div style={{ padding: 24, maxWidth: '100%', margin: '0 auto' }}>
      ...
      {onBack && (
        <button
          ...
          style={{
            ...
            color: '#1a234f', fontSize: 12.5, fontWeight: 600,
            cursor: 'pointer',
          }}
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 20 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
              <h2 style={{ fontSize: 24, fontWeight: 800, color: '#16203a', margin: 0 }}>{data.store_name}</h2>
              <span style={{
                display: 'inline-flex', alignItems: 'center', gap: 6,
              ...
              {!data.is_online && data.last_seen_at && (
                <span style={{ fontSize: 11.5, color: '#7e88a0' }}>
            ...
            : (
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={data.hourly_sales_today} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                  ...
                  <XAxis dataKey="hour" tickFormatter={(h) => `${h}:00`} fontSize={11} stroke="#7e88a0" />
                  <YAxis fontSize={11} stroke="#7e88a0" />
                  <Tooltip
              ...
              <ResponsiveContainer width="100%" height={220}>
                <LineChart data={data.daily_sales_last_7} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                  ...
                  <Tooltip
                    ...
                    contentStyle={{ borderRadius: 8, border: '1px solid #e5e7eb', fontSize: 12 }}
                  />
                  <Line type="monotone" dataKey="revenue" stroke="#1f2a63" strokeWidth={2.5} dot={{ r: 3, fill: '#1f2a63' }} />
                </LineChart>
              </ResponsiveContainer>
            )
          }
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <tbody>
                  {data.top_products_today?.map((p, i) => (
                    <tr key={i} style={{ borderBottom: i < (data.top_products_today!.length - 1) ? '1px solid #f1f4fb' : 'none' }}>
                      <td style={{ padding: '10px 4px', fontSize: 13.5, color: '#16203a', fontWeight: 600 }}>
                        <span style={{ display: 'inline-block', width: 20, fontWeight: 700, color: '#7e88a0', fontSize: 12 }}>
                          {i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : `${i + 1}.`}
                        </span>
                        {p.name}
                      </td>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {data.active_sessions?.map((s) => (
                  <div key={s.session_id} style={{
                    ...
                    background: '#f9fafb', borderRadius: 10, border: '1px solid #f1f4fb',
                  }}>
                    <div style={{
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 13.5, fontWeight: 700, color: '#16203a' }}>{s.cashier_name ?? '—'}</div>
                      <div style={{ fontSize: 11.5, color: '#7e88a0' }}>
        ...
        {(data.recent_sales?.length ?? 0) === 0
          ...
          : (
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ background: '#f9fafb', borderBottom: '1px solid #f1f4fb' }}>
                  {[isNl ? 'Tijd' : 'Time', isNl ? 'Bon' : 'Sale #', isNl ? 'Kassier' : 'Cashier', isNl ? 'Betaling' : 'Payment', isNl ? 'Totaal' : 'Total'].map((h) => (
                    <th key={h} style={{ padding: '9px 12px', textAlign: 'left', fontSize: 11, fontWeight: 700, color: '#5f6a84', textTransform: 'uppercase', letterSpacing: '0.4px' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {data.recent_sales?.map((s, i) => (
                  <tr key={s.id} style={{ borderBottom: i < (data.recent_sales!.length - 1) ? '1px solid #f1f4fb' : 'none' }}>
                    <td style={{ padding: '10px 12px', color: '#6b7280', whiteSpace: 'nowrap' }}>{fmtRelHour(s.occurred_at)}</td>
                    <td style={{ padding: '10px 12px', fontFamily: 'monospace', fontSize: 12, color: '#1a234f' }}>{s.sale_number}</td>
                    <td style={{ padding: '10px 12px', color: '#16203a' }}>{s.cashier_name ?? '—'}</td>
                    <td style={{ padding: '10px 12px' }}>
                      <span style={{ padding: '2px 9px', borderRadius: 6, fontSize: 11, fontWeight: 600, background: '#eef2ff', color: '#1a234f' }}>
                        {s.payment_method}
                      </span>
                    </td>
      ...
      <div style={{ marginTop: 24, padding: '14px 18px', background: '#fafbff', borderRadius: 12, border: '1px solid #e9eef9', display: 'flex', gap: 18, flexWrap: 'wrap', alignItems: 'center', fontSize: 12.5, color: '#6b7280' }}>
...
function KpiTile({ label, value, delta, accent }: { label: string; value: string; delta?: string | null; accent: string }) {
  ...
  return (
    <div style={{
      ...
      border: '1px solid #e9eef9', boxShadow: '0 1px 2px rgba(0,0,0,0.02)',
      position: 'relative', overflow: 'hidden',
    }}>
      <div style={{ position: 'absolute', top: 0, left: 0, width: 4, height: '100%', background: accent }} />
      <div style={{ fontSize: 11, fontWeight: 700, color: '#7e88a0', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 6 }}>{label}</div>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
        <div style={{ fontSize: 20, fontWeight: 800, color: '#16203a' }}>{value}</div>
        {delta != null && (
          <div style={{
...
function Card({ title, subtitle, children }: { title: string; subtitle?: string; children: React.ReactNode }) {
  return (
    <div style={{ background: '#fff', borderRadius: 12, padding: '16px 20px', border: '1px solid #e9eef9', boxShadow: '0 1px 2px rgba(0,0,0,0.02)' }}>
      <div style={{ marginBottom: 12 }}>
        <h3 style={{ fontSize: 13, fontWeight: 700, color: '#374151', margin: 0 }}>{title}</h3>
        {subtitle && <p style={{ fontSize: 11.5, color: '#7e88a0', marginTop: 3 }}>{subtitle}</p>}
      </div>
      {children}
    </div>
  )
...
function EmptyPanel({ hint, icon = '📊' }: { hint: string; icon?: string }) {
  return (
    <div style={{ padding: '24px 12px', textAlign: 'center', color: '#7e88a0' }}>
